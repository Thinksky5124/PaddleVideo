...
class ETE(BaseSegmenter):
    ...
    def __init__(self,
        ...
        self.val_sliding_strike = val_sliding_strike
    ...
    def forward_net(self, imgs, seg_mask, start_frame):
        ...
        if self.head is not None:
            seg_score, cls_score = self.head(seg_feature, cls_feature, seg_mask)
        else:
            seg_score = None
    ...
    def train_step(self, data_batch):
        ...
        pred_score = paddle.zeros((imgs_np.shape[0], self.num_classes,
                                   imgs_np.shape[1] * self.sample_rate))
        ...
        for start_frame in range(0, imgs_np.shape[1], self.sliding_strike):
            segment_len = self.sample_rate * self.num_segs * (
                self.clip_buffer_num + 1)
            ...
            if end_frame > imgs_np.shape[1]:
                break
            # generate index
            start_overwrite_frame = segment_len - (
                self.sliding_strike * sliding_cnt +
                self.num_segs) * self.sample_rate
            if start_overwrite_frame < 0:
                start_overwrite_frame = 0
            gt_start_frame = (start_frame - self.clip_buffer_num *
                              self.num_segs) * self.sample_rate
            if gt_start_frame < 0:
                gt_start_frame = 0
            ...
            if seg_gt.shape[1] < segment_len:
                seg_gt_pad = paddle.to_tensor(
                    np.full((seg_gt.shape[0], segment_len - seg_gt.shape[1]),
                            -100,
                            dtype=np.int64))
                seg_gt = paddle.concat([seg_gt_pad, seg_gt], axis=1)
                mask_pad = paddle.to_tensor(
                    np.zeros(
                        (sample_seg_mask.shape[0], sample_seg_mask.shape[1],
                         segment_len - sample_seg_mask.shape[2]),
                        dtype=np.float32))
                sample_seg_mask = paddle.concat([mask_pad, sample_seg_mask],
                                                axis=2)

            # call forward
            headoutputs = self.forward_net(imgs, sample_seg_mask, start_frame)

            output, cls_score = headoutputs

            # step 6 post precessing
            pred_score[:, :, gt_start_frame:gt_end_frame] = output[
                -1, :, :, start_overwrite_frame:].clone().detach()

            seg_loss = 0.
            for i in range(len(output)):
                seg_loss += self.head.seg_loss(output[i], seg_gt,
                                               sample_seg_mask)

            cls_loss = self.head.feature_extract_loss(cls_score, seg_gt)
            loss = (self.seg_weight * seg_loss +
                    self.cls_weight * cls_loss) / sliding_num
            # 4.2 backward
            loss.backward()

            # log loss
            total_seg_loss = total_seg_loss + seg_loss / sliding_num
            total_cls_loss = total_cls_loss + cls_loss / sliding_num
        ...
        pred_score_list, pred_cls_list = self._post_processing(
            pred_score, video_gt)

        loss_metrics = dict()
        loss_metrics['loss'] = (total_seg_loss + total_cls_loss) * sliding_num
        loss_metrics['seg_loss'] = total_seg_loss * sliding_num
        loss_metrics['cls_loss'] = total_cls_loss * sliding_num
        loss_metrics['F1@0.50'] = self.head.get_F1_score(
            pred_cls_list, video_gt)
    ...
    def val_step(self, data_batch):
        ...
        total_seg_loss = 0.
        total_cls_loss = 0.
        sliding_num = imgs_np.shape[1] // self.val_sliding_strike
        if imgs_np.shape[1] % self.val_sliding_strike != 0:
            sliding_num = sliding_num + 1
        sliding_cnt = 0
        # sliding segmentation
        for start_frame in range(0, imgs_np.shape[1], self.val_sliding_strike):
            segment_len = self.sample_rate * self.num_segs * (
                self.clip_buffer_num + 1)
            # step 1 sliding sample
            end_frame = start_frame + self.num_segs
            ...
            # generate index
            start_overwrite_frame = segment_len - (
                self.val_sliding_strike * sliding_cnt +
                self.num_segs) * self.sample_rate
            if start_overwrite_frame < 0:
                start_overwrite_frame = 0
            gt_start_frame = (start_frame - self.clip_buffer_num *
